...
The training logs and model checkpoints are stored in a separate Git repository with Git LFS support.
...
```bash
git clone https://github.com/SURE3187774683/logs.git
cd logs
```

**Shallow clone (faster, no history):**
```bash
git clone --depth 1 https://github.com/SURE3187774683/logs.git
cd logs
```
...
GIT_LFS_SKIP_SMUDGE=1 git clone https://github.com/SURE3187774683/logs.git
cd logs

# Download only specific files (e.g., only .pt model files in a specific directory)
git lfs pull --include="field_G1/Dec17_03-40-37_V1_5terrain_pro_CNN+BatchNorm/*.pt"

# Or download all LFS files later
git lfs pull
```

**Note:** The repository contains approximately **107+ GB** of LFS data. Ensure you have sufficient disk space and network bandwidth.
